...
public class FolderContentsTests
{
    ...
    {
        var dir = Path.Combine(Path.GetTempPath(), "MSDeskFC_" + Guid.NewGuid().ToString("N"));
    ...
    {
        var missing = Path.Combine(Path.GetTempPath(), "MSDeskFC_missing_" + Guid.NewGuid().ToString("N"));
